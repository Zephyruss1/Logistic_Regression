...
def test_dtype_data(setup_data):
  ...
  assert x_train.dtype == np.float32 and x_test.dtype == np.float32, "Data type is not `float32`"
  assert y_train.dtype == np.uint8 and y_test.dtype == np.uint8, "Data type is not `uint8`"
