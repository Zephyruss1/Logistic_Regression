...
class XGBoostModel:
  ...
  def __init__(self, params, X, y, random_seed=None):
    ...
    self.subsample = self.params["subsample"] if self.params["subsample"] is not None else 1.0
    self.learning_rate = self.params["learning_rate"] if self.params["learning_rate"] is not None else 0.3
    self.base_prediction = self.params["base_score"] if self.params["base_score"] is not None else 0.5
    self.max_depth = self.params["max_depth"] if self.params["max_depth"] is not None else 5
  ...
  def fit(self, objective, num_boost_round, verboose=False):
    ...
    for i in range(num_boost_round):
      ...
      sample_idxs = (
        ...
        else self.rng.choice(len(self.y), size=int(np.floor(self.subsample * len(self.y))), replace=False)
      )
      booster = TreeBooster(self.X, gradients, hessians, self.params, self.max_depth, sample_idxs)
...
class TreeBooster:
  ...
  def __init__(self, X, g, h, params, max_depth, idxs=None):
    ...
    self.min_child_weight = params["min_child_weight"] if params["min_child_weight"] is not None else 1.0
    self.reg_lambda = params["reg_lambda"] if params["reg_lambda"] is not None else 1.0
    self.gamma = params["gamma"] if params["gamma"] is not None else 0.0
    self.colsample_bynode = params["colsample_bynode"] if params["colsample_bynode"] is not None else 1.0
