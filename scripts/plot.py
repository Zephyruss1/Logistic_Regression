...
def load_result(name: str):
  ...
  if not path.exists():
    raise FileNotFoundError(f"[PLOT WARNING]: '{path.name}' not found. Run main.py with optimizer='{name}' first.")
...
def plot_logreg():
  ...
  if args.comparison == 1:
    try:
      results = {name: load_result(name) for name in OPTIMIZER_PKL_FILES}
    except FileNotFoundError:
      raise FileNotFoundError("[PLOT WARNING]: File not found.")
    except Exception as e:
      raise ValueError(
        f"[PLOT ERROR]:If you are running this script alone,\
  ...
  try:
    ...
    plot_comparison(
      ...
      "Objective Index",
      "Objective Value",
      "comparison_objective.png",
    )
  except FileNotFoundError:
    raise FileNotFoundError("File not found. Please run main.py first with relating optimization.")
  except Exception as e:
    raise ValueError(f"[PLOT ERROR]: Error occurred while plotting: {e}")

  if args.optimizer not in results:
